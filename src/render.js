...
function localTerrainSVG(s,p,palette,view){
 const lang=L.language(view),t=s.terrain,key=lang+':'+palette+':local:'+t.revision+':'+view.terrainDisplay+':'+view.contours+':'+view.hillshade+':'+s.options.forest;
 ...
 out+=`<g id="elevation-legend" pointer-events="none"><rect x="26" y="26" width="275" height="${view.terrainDisplay==='elevation'?82:62}" rx="5" fill="${p.paper}" opacity=".9"/><text x="38" y="45" font-family="system-ui,sans-serif" font-size="10" fill="${p.ink}">${lang==='it'?'RILIEVO LOCALE':'LOCAL SURVEY'} · ${s.scale} × ${s.scale} km</text><text x="38" y="62" font-family="system-ui,sans-serif" font-size="10" fill="${p.ink}">${Math.round(minM)}–${Math.round(maxM)} m · ${lang==='it'?'media':'mean'} ${Math.round(t.meanM)} m</text><text x="38" y="78" font-family="system-ui,sans-serif" font-size="9" fill="${p.ink}">${interval?(lang==='it'?'Curve di livello ':'Contours ')+Number(interval.toPrecision(3))+' m · ':''}${lang==='it'?'Campioni':'Samples'} ${Number(t.cellMeters.toPrecision(4))} m</text>`;
...
function render(s,view={}){const lang=L.language(view);view={...C.appearance(s),...view};const palette=view.palette||'atlas',high=HQ.enabled(s,view),p=high?HQ.palette(palettes[palette]||palettes.atlas,palette):palettes[palette]||palettes.atlas,layers={terrain:true,water:true,roads:true,buildings:true,districts:false,vegetation:true,assets:true,labels:true,...view.layers},hide=f=>f.hidden||view.player&&f.gmOnly||!layers[layer(f)]||City.hidden(f,s,view),w=s.width,h=s.height;
 ...
 case'room':break;
 case'label':shape=`<text x="${num(f.x)}" y="${num(f.y)}" text-anchor="middle" font-family="Georgia,serif" font-size="${num(f.size||16)}" transform="rotate(${num(f.rotation||0)} ${num(f.x)} ${num(f.y)})" fill="${p.ink}" stroke="${p.paper}" stroke-width="3" paint-order="stroke">${esc(L.feature(f,lang))}</text>`;break;
 ...
 if(layers.labels){for(const f of fs){if(s.cityStudio&&f.cityComplex&&!f.labelCustom&&(f.cityRole==='civic'&&f.cityComplexKind!=='temple'||f.cityRole==='complex-ground'&&f.cityComplexKind==='temple'))continue;if(hide(f)||!f.label||['building','asset','label','decoration','image'].includes(f.type)&&!(s.cityStudio&&f.cityRole==='civic'))continue;if(f.type==='district'&&(!layers.districts||!s.cityStudio&&parseInt(f.id.slice(1),10)%3!==0))continue;if(s.cityStudio&&['road','area','wall'].includes(f.type))continue;const anchor=C.center(f);const size=f.type==='room'&&s.battle?.generatorVersion===2?E.clamp(s.gridSize*.34,5,13):f.type==='district'?10:s.mode==='region'?13:12;out+=`<text data-id="${esc(f.id)}" x="${num(anchor[0])}" y="${num(anchor[1]+(f.type==='room'||f.type==='district'?0:(f.size||10)+11))}" font-size="${size}" text-anchor="middle" font-family="Georgia,serif" fill="${p.ink}" stroke="${p.paper}" stroke-width="2.6" stroke-opacity=".88" paint-order="stroke">${esc(L.feature(f,lang))}</text>`;}}
...
return{labels:L,render,assetPreview,assetFile,symbols,palettes,esc,pts};
});
