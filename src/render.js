...
(function(root,factory){const e=typeof module==='object'&&module.exports?require('./engine.js'):root.MegamapEngine;const a=typeof module==='object'&&module.exports?require('./assets.js'):root.MegamapAssets,c=typeof module==='object'&&module.exports?require('./editor-core.js'):root.MegamapCore;const b=typeof module==='object'&&module.exports?require('./battle-render.js'):root.MegamapBattleRender;const api=factory(e,a,c,b);if(typeof module==='object'&&module.exports)module.exports=api;else root.MegamapRender=api;})(typeof globalThis!=='undefined'?globalThis:this,function(E,A,C,B){
'use strict';
...
function render(s,view={}){view={...C.appearance(s),...view};const palette=view.palette||'atlas',p=palettes[palette]||palettes.atlas,layers={terrain:true,water:true,roads:true,buildings:true,districts:false,vegetation:true,assets:true,labels:true,...view.layers},hide=f=>f.hidden||view.player&&f.gmOnly||!layers[layer(f)],w=s.width,h=s.height;
 ...
 if(s.battle&&layers.roads){let walls=C.mergeWalls(E.wallSegments(s));for(const f of fs.filter(f=>f.type==='portal'&&!hide(f)))walls=walls.flatMap(l=>C.cutDoor(l,f.points,s.gridSize*.04));out+=s.battle.generatorVersion===2?B.walls(s,p,walls):`<path d="${walls.map(l=>`M${pts([l[0]])}L${pts([l[1]])}`).join('')}" stroke="${p.ink}" stroke-width="${num(s.gridSize*.12)}" fill="none" pointer-events="none"/>`;}
