...
function icon(f){const size=f.size||15,x=f.x||0,y=f.y||0;return `<use href="#asset-${E.ASSETS.includes(f.asset)?f.asset:'rock'}" x="${num(x-size)}" y="${num(y-size)}" width="${num(size*2)}" height="${num(size*2)}" transform="rotate(${num(f.rotation||0)} ${num(x)} ${num(y)})"/>`;}
...
function render(s,view={}){view={...C.appearance(s),...view};const palette=view.palette||'atlas',p=palettes[palette]||palettes.atlas,layers={terrain:true,water:true,roads:true,buildings:true,districts:false,vegetation:true,assets:true,labels:true,...view.layers},hide=f=>f.hidden||view.player&&f.gmOnly||!layers[layer(f)],w=s.width,h=s.height;
 ...
 if(s.mode==='city'&&layers.terrain){out+=`<rect width="${w}" height="${h}" fill="${p.land}"/><rect x="45" y="45" width="910" height="910" fill="url(#paper-hatch)" opacity=".4"/>`;}
 ...
 if(s.battle&&view.textures!==false&&!['forest','bridge','desert'].includes(s.options.theme)){
 ...
 for(const f of fs){if(hide(f))continue;let shape='',poly=f.polygon?pts(f.polygon):'',line=f.points?pts(f.points):'';
 switch(f.type){case'district':{const colors=['#ccb89b','#c3cba8','#d6b79b','#b7c9ba','#cfc3a5'];shape=`<polygon points="${poly}" fill="${colors[E.hash(f.ward||f.label)%colors.length]}" fill-opacity=".65" stroke="${p.ink}" stroke-opacity=".45" stroke-width=".8" stroke-dasharray="4 3"/>`;break;}
 case'plaza':shape=`<polygon points="${poly}" fill="${p.sand}" stroke="${p.ink}" stroke-opacity=".25" stroke-width="1"/>`;break;
 case'area':shape=`<polygon points="${poly}" fill="${p[f.material]||p.grass}" stroke="${p.ink}" stroke-width=".6" stroke-opacity=".3"/>`;break;
 ...
 case'portal':shape=`<polyline points="${line}" fill="none" stroke="${p.ink}" stroke-width="6"/><polyline points="${line}" fill="none" stroke="${p.roof[1]}" stroke-width="4"/>`;break;
 ...
 if(s.battle&&layers.roads){let walls=C.mergeWalls(E.wallSegments(s));for(const f of fs.filter(f=>f.type==='portal'&&!hide(f)))walls=walls.flatMap(l=>C.cutDoor(l,f.points,s.gridSize*.04));out+=`<path d="${walls.map(l=>`M${pts([l[0]])}L${pts([l[1]])}`).join('')}" stroke="${p.ink}" stroke-width="${num(s.gridSize*.12)}" fill="none" pointer-events="none"/>`;}
