...
function mount(doc,I){if(instance)return instance;const P=root.MegamapCityPlan,win=doc.defaultView,t=x=>I?.getLang()==='it'?(IT[x]||I.t(x)):x;
 ...
 const previewButton=button('Preview city',makePreview,'cityPaintPreviewButton'),back=button('Back to painting',()=>{preview.hidden=true;canvas.hidden=false;back.hidden=true;perspective.hidden=true;render();},'cityPaintBack');back.hidden=true;const perspective=button('2.5D preview',()=>{if(previewScene)root.MegamapCityView.open(previewScene,previewScene.appearance);},'cityPaint25D');perspective.hidden=true;actions.append(sample,clear,restore,previewButton,back,perspective);stage.append(actions);
 ...
 function makePreview(){if(previewing)return;endStroke();if(!enable.checked)enable.checked=true;const before=revision,opts=options();opts.cityPlan=plan();previewing=true;previewButton.disabled=true;status.textContent=t('Generating preview…');
  win.setTimeout(()=>{try{const scene=root.MegamapEngine.generate('fantasy',doc.getElementById('seed').value||'city-plan',opts);root.MegamapEngine.validateScene(scene);if(before!==revision)return;previewScene=scene;preview.innerHTML=root.MegamapRender.render(scene,{labels:false});preview.hidden=false;canvas.hidden=true;back.hidden=false;perspective.hidden=false;status.textContent=scene.cityStudio.statistics.buildings+' '+t('buildings')+' · '+t('Preview uses the current preset, size and climate; it does not add a map.')+(scene.cityStudio.warnings.length?' '+scene.cityStudio.warnings.map(w=>t(w)).join(' '):'');}catch(e){status.textContent=t(e.message);canvas.hidden=false;preview.hidden=true;}finally{previewing=false;previewButton.disabled=false;}},30);
 }
 function language(){if(previewScene&&!preview.hidden)preview.innerHTML=root.MegamapRender.render(previewScene,{labels:false});node.querySelectorAll('[data-copy]').forEach(x=>x.textContent=t(x.dataset.copy));canvas.setAttribute('aria-label',t('Painted plan canvas'));size.setAttribute('aria-label',t('Brush size'));render();}
