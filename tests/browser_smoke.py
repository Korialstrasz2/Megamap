...
with sync_playwright() as p:
    ...
    try:
        ...
        check('Startup: version, default 20 km region, 304 assets and visible Generate button',lambda:(expect(page.evaluate('MegamapApp.getVersion()')=='1.2.0'),expect(s()['scale']==20),expect(page.locator('.asset-card').count()==304),expect(page.locator('#generate').is_visible())))
        def styles():
